...
async function signup(req, res, next) {
    try {
        ...
        if ([username, email, password].some((val) => !val?.trim())) {
            return next(new ApiError_1.ApiError("All fields are required", 400));
        }
        ...
        if (existingUser) {
            return next(new ApiError_1.ApiError("user already exists", 409));
        }
        //Handle avatar upload
        const avatarImage = req.file?.buffer;
        let imageUrl;
        if (avatarImage) {
            imageUrl = await (0, Cloudinary_1.UploadToCloudinary)(avatarImage);
        }
        //Create new user
        const newUser = await user_models_1.User.create({
            username,
            email,
            password,
        ...
        if (!newUser) {
            return next(new ApiError_1.ApiError("Error while register user", 400));
        }
        ...
        await emailQueue_1.default.add("sendEmail", { options }, { attempts: 2, backoff: 5000 });
        //Send sms to user
        const number = req.body?.mobile;
        if (number) {
            //add sms job to queue
            await smsQueue_1.default.add("sendSms", { number }, { attempts: 2 });
        }
        const userObj = {
...
async function verifyEmail(req, res, next) {
    try {
        ...
        if (!user) {
            return next(new ApiError_1.ApiError("Invalid or expired token", 400));
        }
        if (user.isVerified) {
            return next(new ApiError_1.ApiError("User is verified", 409));
        }
        ...
        await redisClient_1.publisher.publish("userSignup", JSON.stringify(options));
        res
            .status(200)
...
async function resendEmail(req, res, next) {
    try {
        ...
        const existingUser = await user_models_1.User.findOne({ email: email });
        if (!existingUser) {
            return next(new ApiError_1.ApiError("User is not found", 404));
        }
        ...
        await emailQueue_1.default.add("sendEmail", { options });
        res
            .status(200)
            .json({ success: true, message: "Email verified successfully" });
    }
...
async function signin(req, res, next) {
    try {
        ...
        if (!UsernameOrEmail || !password) {
            return next(new ApiError_1.ApiError("please give  proper values", 400));
        }
        //Handle existing user
        const existingUser = await user_models_1.User.findOne({
        ...
        if (!existingUser) {
            return next(new ApiError_1.ApiError("User does not exist please signup first", 400));
        }
        //Compare password using bcrypt
        const passwordCompare = await existingUser.comparePassword(password);
        if (!passwordCompare) {
            return next(new ApiError_1.ApiError("Password is not correct", 400));
        }
